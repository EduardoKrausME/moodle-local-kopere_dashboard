...
$akey = optional_param('akey', '', PARAM_RAW);

if (USE_ACCESS_KEYS == true) {
    if (!isset($akey, $access_keys) || empty($access_keys)) {
        die('Access Denied!');
    }

    $akey = strip_tags(preg_replace("/[^a-zA-Z0-9\._-]/", '', $akey));

    if (!in_array($akey, $access_keys)) {
        die('Access Denied!');
    }
...
if (optional_param('submit', false, PARAM_RAW)) {
    ...
    if (optional_param('fldr', false, PARAM_RAW)
        && !empty(optional_param('fldr', '', PARAM_RAW))
        && strpos(optional_param('fldr', '', PARAM_RAW), '../') === false
        && strpos(optional_param('fldr', '', PARAM_RAW), './') === false
    ) {
        $subdir = urldecode(trim(strip_tags(optional_param('fldr', '', PARAM_RAW)), "/") . "/");
    } else {
        $subdir = '';
    }
    ...
    if (optional_param('popup', false, PARAM_RAW)) {
        $popup = strip_tags(optional_param('popup', '', PARAM_RAW));
    } else $popup = 0;
    //Sanitize popup
    $popup = !!$popup;
    ...
    if (optional_param('view', false, PARAM_RAW)) {
        $view = fix_get_params(optional_param('view', '', PARAM_RAW));
        $_SESSION['RF']["view_type"] = $view;
    }
    ...
    if (optional_param('lang', false, PARAM_RAW) && optional_param('lang', '', PARAM_RAW) != 'undefined' && optional_param('lang', '', PARAM_RAW) != '') {
        $lang = fix_get_params(optional_param('lang', '', PARAM_RAW));
        $lang = trim($lang);
    }
    ...
    $field_id = optional_param('field_id', false, PARAM_RAW) ? fix_get_params(optional_param('field_id', '', PARAM_RAW)) : '';
    $type_param = fix_get_params(optional_param('type', '', PARAM_RAW));
    ...
    <head>
        ...
        <link href="css/group_css.css" rel="stylesheet" type="text/css"/>
        <?php
    ...
    <div class="container-fluid">
        ...
        if (optional_param('type', '', PARAM_RAW) == 1) {
            $apply = 'apply_img';
        } else if (optional_param('type', '', PARAM_RAW) == 2) {
            $apply = 'apply_link';
        } else if (optional_param('type', '', PARAM_RAW) == 0 && optional_param('field_id', '', PARAM_RAW) == '') {
            $apply = 'apply_none';
        } else if (optional_param('type', '', PARAM_RAW) == 3) {
            $apply = 'apply_video';
        } else {
            $apply = 'apply';
        }
        ...
        foreach ($files as $k => $file) {
            ...
            else if (is_dir($current_path . $rfm_subfolder . $subdir . $file)) {
                        ...
                        <div class="filters">
                            <div class="row-fluid">
                                ...
                                <div class="span6 half types">
                                    ...
                                    <?php if (optional_param('type', '', PARAM_RAW) != 1 && optional_param('type', '', PARAM_RAW) != 3) { ?>
                                    ...
                                    <input accesskey="f" type="text"
                                           class="filter-input <?php echo((optional_param('type', '', PARAM_RAW) != 1 && optional_param('type', '', PARAM_RAW) != 3) ? '' : 'filter-input-notype'); ?>"
                                    ...
                                    <input id="select-type-all" name="radio-sort" type="radio"
                                           data-item="ff-item-type-all"
                                           class="hide"/>
                                    <label id="ff-item-type-all" title="<?php echo lang_All; ?>"
                                           <?php if (optional_param('type', '', PARAM_RAW) == 1 || optional_param('type', '', PARAM_RAW) == 3){ ?>style="visibility: hidden;" <?php } ?>
                ...
                <ul class="grid cs-style-2 <?php echo "list-view" . $view; ?>" id="main-item-container">
                    ...
                    foreach ($files as $file_array) {
                        ...
                        if ($file == '..') {
                            ...
                            ?>
                            <figure data-name="<?php echo $file ?>" class="<?php
                            if ($file == "..") echo "back-"; ?>directory"
                                ...
                                <?php } else { ?>
                                    ...
                                    <?php if ($show_folder_size) { ?>
                                        <div
                                                class="file-size"><?php echo makeSize($file_array['size']) ?></div><?php } ?>
                    ...
                    if (in_array($extension_lower, $ext_img)) {
                        $class_ext = 2;
                    } else if (in_array($extension_lower, $ext_misc)) {
                        $class_ext = 3;
                    } else {
                        $class_ext = 1;
                    }

                    $src_thumb = str_replace($root_dir, $root_href, $src_thumb);
                    $mini_src = str_replace($root_dir, $root_href, $mini_src);


                    if ((!(optional_param('type', '', PARAM_RAW) == 1 && !$is_img) && !((optional_param('type', '', PARAM_RAW) == 3 && !$is_video) && (optional_param('type', '', PARAM_RAW) == 3 && !$is_audio))) && $class_ext > 0){
                    ?>
