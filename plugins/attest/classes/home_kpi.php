...
class home_kpi {
    ...
    public static function get_metric(context $context): ?array {
        ...
        if (!self::table_exists("koperedashboard_attest_issue")) {
            return null;
        }
        ...
        return [
            "title" => get_string("home_kpi_title", "koperedashboard_attest"),
            "value" => self::format_number((int) $value),
            "subtitle" => get_string("home_kpi_subtitle", "koperedashboard_attest"),
            "url" => (new moodle_url("/local/kopere_dashboard/plugins/attest/manage.php"))->out(false),
            "style" => "kopere_dashboard-kpi-cyan",
        ];
    }
