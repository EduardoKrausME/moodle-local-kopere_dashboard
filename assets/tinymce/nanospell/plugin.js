 ...
 *		nanospell_dictionary: 'pt_br', // download 23 more at http://tinymcespellcheck.com/dictionaries
...
(function(exports, undefined) {
	...
	PluginManager.add('nanospell', function(editor, url) {
		...
		function getMenuItemsArray(target, word) {
			...
			if (!items.length) {
				items.push({
					text: "(Não há sugestões de ortografia)",
					disabled: true
				});
			}
			if (suggestions && suggestions.length == 2 && suggestions[0].indexOf(String.fromCharCode(160)) > -1) { /****/ } else {
				...
				items.push({
					text: 'Adicionar ao dicionário pessoal',
					onclick: function() {
						addPersonal(word);
						ignoreWord(target, word, true);
					}
