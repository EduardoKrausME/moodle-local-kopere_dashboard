Vvveb.ComponentsGroup["Widgets"]=["widgets/googlemaps","widgets/embed-video"];Vvveb.Components.extend("_base","widgets/googlemaps",{name:"Google Maps",attributes:["data-component-maps"],image:"icons/map.svg",dragHtml:'<img src="'+Vvveb.baseUrl+'icons/maps.png">',html:'<div data-component-maps><iframe frameborder="0" src="https://maps.google.com/maps?q=Bucharest&z=15&t=q&key=&output=embed" width="100%" height="100%" style="width:100%;height:100%;left:0"></iframe></div>',resizable:true,resizeMode:"css",z:3,q:"Paris",t:"q",key:"",init:function(node){let iframe=node.querySelector("iframe");let url=new URL(iframe.getAttribute("src"));let params=new URLSearchParams(url.search);this.z=params.get("z");this.q=params.get("q");this.t=params.get("t");this.key=params.get("key");document.querySelector(".component-properties input[name=z]").value=this.z;document.querySelector(".component-properties input[name=q]").value=this.q;document.querySelector(".component-properties select[name=t]").value=this.t;document.querySelector(".component-properties input[name=key]").value=this.key},onChange:function(node,property,value){map_iframe=node.querySelector("iframe");this[property.key]=value;mapurl="https://maps.google.com/maps?q="+this.q+"&z="+this.z+"&t="+this.t+"&output=embed";map_iframe.setAttribute("src",mapurl);return node},properties:[{name:"Address",key:"q",inputtype:TextInput},{name:"Map type",key:"t",inputtype:SelectInput,data:{options:[{value:"q",text:"Roadmap"},{value:"w",text:"Satellite"}]}},{name:"Zoom",key:"z",inputtype:RangeInput,data:{max:20,min:1,step:1}},{name:"Key",key:"key",inputtype:TextInput}]});Vvveb.Components.extend("_base","widgets/openstreetmap",{name:"Open Street Map",attributes:["data-component-openstreetmap"],image:"icons/map.svg",dragHtml:'<img src="'+Vvveb.baseUrl+'icons/maps.png">',html:`<div data-component-openstreetmap><iframe width="100%" height="100%" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="https://www.openstreetmap.org/export/embed.html?bbox=-62.04673002474011%2C16.95487694424327%2C-61.60521696321666%2C17.196751341562923&layer=mapnik"></iframe></div>`,resizable:true,resizeMode:"css",bbox:"",layer:"mapnik",init:function(node){let iframe=node.querySelector("iframe");let url=new URL(iframe.getAttribute("src"));let params=new URLSearchParams(url.search);this.bbox=params.get("bbox");this.layer=params.get("layer");document.querySelector(".component-properties input[name=bbox]").value=this.bbox;document.querySelector(".component-properties input[name=layer]").value=this.layer},onChange:function(node,property,value){map_iframe=node.querySelector("iframe");this[property.key]=value;mapurl="https://www.openstreetmap.org/export/embed.html?bbox="+this.bbox+"&layer="+this.layer;map_iframe.setAttribute("src",mapurl);return node},properties:[{name:"Map",key:"bbox",inputtype:TextInput}]});Vvveb.Components.extend("_base","widgets/embed-video",{name:"Embed Video",attributes:["data-component-video"],image:"icons/youtube.svg",dragHtml:'<img src="'+Vvveb.baseUrl+'icons/youtube.svg" width="100" height="100">',html:'<div data-component-video style="width:640px;height:480px;"><iframe frameborder="0" src="https://player.vimeo.com/video/24253126?autoplay=false&controls=false&loop=false&playsinline=true&muted=false" width="100%" height="100%"></iframe></div>',t:"y",video_id:"",url:"",autoplay:false,controls:false,loop:false,playsinline:true,muted:false,resizable:true,resizeMode:"css",youtubeRegex:/(?:youtube\.com\/(?:[^\/]+\/.+\/|(?:v|e(?:mbed)?)\/|.*[?&]v=)|youtu\.be\/)([^"&?\/\s]+)/i,vimeoRegex:/(?:vimeo\.com(?:[^\d]+))(\d+)/i,init:function(node){iframe=node.querySelector("iframe");video=node.querySelector("video");document.querySelector(".component-properties [data-key=url]").style.display="none";document.querySelector(".component-properties [data-key=poster]").style.display="none";if(video){this.url=video.src}else if(iframe){let src=iframe.getAttribute("src");let match;if(src&&src.indexOf("youtube")&&(match=src.match(this.youtubeRegex))){this.video_id=match[1];this.t="y"}else if(src&&src.indexOf("vimeo")&&(match=src.match(this.vimeoRegex))){this.video_id=match[1];this.t="v"}else{this.t="h"}}document.querySelector(".component-properties input[name=video_id]").value=this.video_id;document.querySelector(".component-properties input[name=url]").value=this.url;document.querySelector(".component-properties select[name=t]").value=this.t},onChange:function(node,property,value){this[property.key]=value;{switch(this.t){case"y":document.querySelector(".component-properties [data-key=video_id]").style.display="";document.querySelector(".component-properties [data-key=url]").style.display="none";document.querySelector(".component-properties [data-key=poster]").style.display="none";newnode=generateElements(`<iframe width="100%" height="100%" allowfullscreen="true" frameborder="0" allow="autoplay"
                                ...
                                </iframe>`)[0];break;case"v":document.querySelector(".component-properties [data-key=video_id]").style.display="";document.querySelector(".component-properties [data-key=url]").style.display="none";document.querySelector(".component-properties [data-key=poster]").style.display="none";newnode=generateElements(`<iframe width="100%" height="100%" allowfullscreen="true" frameborder="0" allow="autoplay"
        ...
        if (document.readyState !== 'loading') {
          ...
          }`;body.appendChild(lib);body.appendChild(code);lib.addEventListener("load",function(){Vvveb.Builder.iframe.contentWindow.initLottie()})}else{Vvveb.Builder.iframe.contentWindow.initLottie(true)}return node}Vvveb.Components.add("widgets/lottie",{name:"Lottie",image:"icons/lottie.svg",attributes:["data-component-lottie"],html:`
